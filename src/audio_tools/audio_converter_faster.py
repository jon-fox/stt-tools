...
def initialize_model_pool(device="cuda"):
    ...
    for _ in range(NUMBER_OF_MODELS):
        ...
        if os.path.isfile(model_file_path):
            ...
            whisper_model = whisperx.load_model("tiny", device=device)
...
def process_audio_segment(index, audio_segment):
    ...
    if min_ms == float('inf') and max_ms == float('-inf'):
        ...
        with open(f"{output_dir}/transcript_{index}_logging.json", "w", encoding="utf-8") as file:
            logger.info(f"Logging keyword segments to {output_dir}/transcript_{index}_logging.json")
